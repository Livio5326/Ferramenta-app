...
MAP_FILE = BASE / "stanley_correzioni_aggiuntive_v3.json"
...
# Backup solo prodotti Stanley prima della modifica
stanley = list(products.find({"marca": {"$regex": "stanley", "$options": "i"}}))
backup = BACKUP_DIR / ("backup_prima_correzione_stanley_descrizioni_v3_" + datetime.now().strftime("%Y%m%d-%H%M") + ".json")
...
# Controllo residui molto sospetti
sospetto_re = re.compile(r"^(\(|\d|[A-Z0-9,./°\-\s]+\)?\s*€?$)|PESO|DOTAZIONE|COLORE|DENTI/POLL|ATTACCO|DIMENSIONI|NR BOLLE|G/K|MM\)|KG\)|CM\)", re.I)
residui = []
for p in products.find({"marca": {"$regex": "stanley", "$options": "i"}}, {"codice_prodotto":1, "barcode":1, "descrizione":1, "categoria":1}).limit(5000):
    d = str(p.get("descrizione") or "").strip()
    if d and sospetto_re.search(d):
        residui.append(p)

report = BACKUP_DIR / ("report_residui_descrizioni_stanley_v3_" + datetime.now().strftime("%Y%m%d-%H%M") + ".txt")
