...
async def mark_pending_invoice_product_imported(pending_id: str | None = None, codice_prodotto: str | None = None, barcode: str | None = None):
    filtro = {"$or": []}

    if pending_id:
        filtro["$or"].append({"id": pending_id})
        filtro["$or"].append({"_id": pending_id})

    if codice_prodotto:
        filtro["$or"].append({"codice_prodotto": codice_prodotto})
        filtro["$or"].append({"codice": codice_prodotto})

    if barcode:
        filtro["$or"].append({"barcode": barcode})
...
async def reconcile_invoice_import(numero_fattura: str):
    """
    Ricalcola lo stato reale di una fattura:
    - un pending è importato se trova un prodotto nel catalogo
      per barcode, codice prodotto/codice, oppure descrizione normalizzata.
    - aggiorna pending_invoice_products
    - aggiorna invoice_imports con conteggi coerenti.
    """
    ...
    pendenti = await db.pending_invoice_products.find({
        "numero_fattura": numero_fattura
    }).to_list(5000)
    ...
    for p in pendenti:
        ...
        codice = str(
            p.get("codice_prodotto")
            or p.get("codice")
            or p.get("codice_fornitore")
            or ""
        ).strip()
        descrizione = str(p.get("descrizione") or "").strip()

        prodotto = None

        # 1. Match forte: barcode
        if barcode:
            prodotto = await db.products.find_one({"barcode": barcode})

        # 2. Match forte: codice prodotto / codice
        if not prodotto and codice:
        ...
        # 3. Match descrizione esatta normalizzata
        if not prodotto and descrizione:
            descr_norm = _norm_match_text(descrizione)

            # Prima prova con campo descrizione normale
            candidates = await db.products.find({
                "descrizione": {"$exists": True, "$ne": ""}
            }).to_list(10000)

            for c in candidates:
                if _norm_match_text(c.get("descrizione")) == descr_norm:
                    prodotto = c
                    break
            ...
            await db.pending_invoice_products.update_one(
                {"_id": p["_id"]},
                {
                    "$set": {
                        ...
                        "risolto": True,
                        "da_importare": False,
                        "matched_product_id": str(prodotto.get("id") or prodotto.get("_id")),
                        "matched_by": (
                            "barcode" if barcode and str(prodotto.get("barcode") or "") == barcode
                            else "codice" if codice
                            else "descrizione"
                        ),
    ...
    await db.invoice_imports.update_many(
        ...
        },
        {
            "$set": {
                ...
                "barcode_non_trovati": non_trovati,
...
async def reconcile_all_invoice_imports():
    ...
    for f in fatture:
        ...
        results.append(await reconcile_invoice_import(numero))
